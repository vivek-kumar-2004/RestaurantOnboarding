...
import EditSpace from './EditSapce';
...
function SpaceDetails() {
    ...
    const handleProductDelete = async (productId) => {
        if (window.confirm("Are you sure you want to delete this space?")) {
            try {
                await axios.delete(`/api/restaurant/deleteSpace/${productId}`);
      ...
      <div className='pt-5 bg-gray-100'>
          ...
          <div className="flex  p-10 pt-5 gap-8">
            <div className="w-1/2 bg-white p-8 rounded-lg shadow-2xl h-[83vh]">
                <h1 className="text-2xl font-bold text-gray-800 mb-6 ">SPACE DETAILS</h1>
                <div className="w-full mb-4">
                    <img className="w-full h-[45vh] object-cover shadow-md" src={product.imageUrl} alt={product.spaceTitle} />
                </div>
                <h2 className="text-xl font-semibold text-gray-600 mb-4"><span className='font-bold text-black text-2xl text-m mr-1'>Space Name:</span>{product.spaceTitle}</h2>
            ...
            <EditSpace onEditComplete={handleEditComplete}/>
        </div>
    </div>
